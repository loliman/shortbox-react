...
function IssueDetailsUS(props) {
  return <IssueDetails bottom={<Bottom {...props} />} details={<Details />} subheader />;
}
  ...
  return (
    <React.Fragment>
      ...
      <Typography>
        Informationen über&nbsp;
        <a
          href={generateMarvelDbUrl(props.issue)}
  ...
  return (
    <div className="usStoryContainer">
      <div className="usStoryDetails">
        ...
        {(props.item.parent
          ? props.item.parent.appearances.length
          : props.item.appearances.length) > 0 ? (
          <React.Fragment>
            <br />
            ...
            <ListItem className="issueStoryIssueItem">
              <div>
                <div className="headingContainer">
                  <Typography className="issueStoryIssue">
                    {generateLabel(props.item.reprintOf.issue.series) +
                      " #" +
                      props.item.reprintOf.issue.number}
                  </Typography>
              ...
              <CoverTooltip
                issue={props.item.reprintOf.issue}
                us={props.us}
                number={props.item.reprintOf.number}
              >
                <IconButton
                  className="detailsIcon issueStoryIssueButton"
                  onMouseDown={(e) =>
                    props.navigate(e, generateUrl(props.item.reprintOf.issue, true), {
                      expand: props.item.reprintOf.number,
                      filter: null,
                    })
                  }
                  aria-label="Details"
          ...
          <List className="issueStoryIssueList">
            {story.reprints.map((child, idx) => {
              ...
              return (
                <ListItem
                  key={idx}
                  className="issueStoryIssueItem"
                  divider={story.reprints.length - 1 !== idx}
                >
                  <div>
                    <div className="headingContainer">
                      <Typography className="issueStoryIssue">
                        {generateLabel(child.issue.series) + " #" + child.issue.number}
                      </Typography>
                      <Typography className="parentTitle">
                  ...
                  <CoverTooltip issue={child.issue} us={props.us} number={child.number}>
                    <IconButton
                      className="detailsIcon issueStoryIssueButton"
                      onMouseDown={(e) =>
                        props.navigate(e, generateUrl(child.issue, true), {
                          expand: child.number,
                          filter: null,
                        })
                      }
            ...
            {props.item.children.map((child, idx) => {
              ...
              return (
                <ListItem
                  key={idx}
                  className="issueStoryIssueItem"
                  divider={props.item.children.length - 1 !== idx}
                >
                  <div>
                    <div className="headingContainer">
                      <Typography className="issueStoryIssue">
                        {generateLabel(child.issue.series) + " #" + child.issue.number}{" "}
                        {child.issue.title ? "- " + child.issue.title : ""}
                      </Typography>
                    ...
                    child.parent.issue.series.volume === props.issue.series.volume ? null : (
                      <CoverTooltip
                      ...
                      >
                        <Typography
                          ...
                          onMouseDown={(e) =>
                            props.navigate(e, generateUrl(child.parent.issue, true), {
                              expand: child.parent.number,
                              filter: null,
                            })
                          }
                        >
                          ...
                          <span className="asLink">
                            {generateLabel(child.parent.issue.series) +
                              " #" +
                    ...
                    {addinfoText !== "" ? (
                      <Typography className="parentTitle">{addinfoText}</Typography>
                    ) : null}
                  </div>

                  <div>
                    {child.issue.collected && props.session ? (
                      <img
                    ...
                    <CoverTooltip issue={child.issue} us={props.us} number={child.number}>
                      <IconButton
                        className="detailsIcon issueStoryIssueButton"
                        onMouseDown={(e) =>
                          props.navigate(e, generateUrl(child.issue, props.us), {
                            expand: child.number,
                            filter: null,
                          })
                        }

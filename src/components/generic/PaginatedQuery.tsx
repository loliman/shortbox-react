...
function PaginatedQuery(props) {
  ...
  const queryName = queryNameRaw[0].toLowerCase() + queryNameRaw.slice(1);
  ...
  const fetchMoreVars = React.useMemo(() => {
    const next = JSON.parse(JSON.stringify(parsedInputVariables || {}));
    if (offsetMode) {
  ...
  const fetchMoreOnScroll = (e, reload) => {
